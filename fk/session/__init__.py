...
class Session(object):
    ...
    def pop(self, request, item, value=True):
        # 获取当前会话
        session_id = get_session_id(request)
        current_session = self.__session_map__.get(
            session_id, {})
        
        # 判断数据项的键是否存在与当前的会话中，如果存在则删除
    ...
    def load_local_session(self):
        ...
        if self.__storage_path__ is not None:
            ...
            for session_id in session_path_list:
                ...
                # 把 Session ID 对应的会话内容绑定添加到会话映射表中
    ...
    def get(self, request, item):
        return self.__session_map__.get(
            get_session_id(request), {}).get(item, None)
