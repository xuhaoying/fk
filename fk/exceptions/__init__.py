...
# 未知处理类型
class UnknownFuncError(FkException):
...
def capture(f):
    def decorator(*args, **options):
        ...
        try:
            # 尝试执行函数
            rep = f(*args, **options)
        except FkException as e:
            # 当捕获到 FkException 这个分类的异常时， 判断下异常的编号
            # 如果不为空且关联在 ERROR_MAP 中，进行对应的处理，反之接着抛出
            if e.code in ERROR_MAP and ERROR_MAP[e.code]:
                # 获取异常关联的结果
                rep = ERROR_MAP[e.code]
                # 如果异常编号小于100，响应状态码统一设置为500服务端错误
                status = int(e.code) if int(e.code) >= 100 else 500
                # 判断结果是否是一个响应体，如果不是，则应该就是自定义异常处理函数，调用它并封装为响应体返回
                return rep if isinstance(rep, Response) or rep is None \
                    else Response(rep(), content_type=content_type, status=status)
            else:
                # 接着抛出没有对应处理的异常
                raise e
        # 返回函数执行正常的结果
...
# 异常处理重载装饰器，参数为异常编号为开发者方便采用整型)
def reload(code):
    def decorator(f):
        # 替换 ERROR_MAP 中异常编号关联的处理逻辑为所装饰的函数
